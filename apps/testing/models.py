...
class test_athlete_interface():
    ...
    def load_all_athletes(data_list):
        for in_dex in data_list:
            ...
            """
            try:  
                #ESTOS 2 ATLETAS SE LE DEBEN ARREGLAR LA FECHA DE NACIMIENTO
                if in_dex['id'] == 2966:
                    print("pillado")
                elif in_dex['id'] == 4051:
                    print("pillado")
                else:
                    pbirthdate = parse_date(in_dex['birthdate'])

                athlete.objects.create(id=in_dex['id'], rut=in_dex['rut'],
                                    first_name=in_dex['firstname'], second_name=in_dex['secondname'],
                                    third_name=in_dex['thirdnames'], four_name=in_dex['fourname'],
                                    flast_name=in_dex['fi_lastnames'], slast_name=in_dex['se_lastnames'], 
                                    thlas_tname=in_dex['th_lastnames'], folast_name=in_dex['fo_lastnames'],
                                    gender=in_dex['sex_id'], birthdate=pbirthdate,
                                    cellphone=in_dex['cellPhone'], email=in_dex['mail'], size=in_dex['size'], height=in_dex['height'], 
                                    region=in_dex['region_id'], club_fk_id=in_dex['club_id'],
                                    created_at=in_dex['created_at'], updated_at=in_dex['updated_at'])
            except Exception as e:
                print("ERROR",e,in_dex['id'])
