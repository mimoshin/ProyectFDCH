...
from base.const import GENDER_CHOICES, LIMIT_ATHLETE_CHOICES, LIMIT_CLUB_CHOICES, CATEGORY_CHOICES, SERIES_CHOICES, TEST_GROUP_CHOICES
...
class championship(models.Model):
    ...
    def categorys_options(self):
        options = []
        print("categorys_options",self.category)
        for x in range(1,9): 
            if self.category[x] == '1':
...
class IAsbtract_Asignation(models.Model):
...
class track_asignation(IAsbtract_Asignation):
...
class jump_asignation(IAsbtract_Asignation):
    mark = models.CharField(max_length=20, default='AUXILIAR')
...
class throw_asignation(IAsbtract_Asignation):
    mark = models.CharField(max_length=20, default='AUXILIAR')
    total_results = models.CharField(max_length=200, default='first;second;third;fourth;five;six')
    total_winds = models.CharField(max_length=200, default='first;second;third;fourth;five;six')
...
class stage_interface():
    ...
    def get_competitions_filter(champ,cate,gend,atle):
        try:
            compts = competition.objects.filter(stage_fk__champ_fk_id=champ,categorys=cate,gender=gend)
            html = '<tr>'
            for x in compts:
                html+= '<td>'+x.test_fk.__str__()+' '+GENDER_CHOICES[x.gender][1]+'</td>\
            ...
            print("ERROR AL CARGAR PRUEBAS DE INSCRIPCION",e)
    ...
    def new_compt(stage_id,data_form):
        try:
            ...
            competition.objects.create(stage_fk_id=stage_id, test_fk_id=data_form['compt'], hour=data_form['time'], 
                                       gender=data_form['gender'],categorys=data_form['category'],round=data_form['round'])
            return True
        except Exception as e:
            print("ERROR AL CREAR UNA COMPETENCIA",e)
            return False
                ...
                try:
                    ...
                    while counter<8:
                        ...
                        if selected.ready == False:
                            if type_test == 0:
                                track_asignation.objects.create(serie_fk_id=new_serie.id,athlete_fk_id=selected.otro.id)
                            elif type_test == 1:
                                jump_asignation.objects.create(serie_fk_id=new_serie.id,athlete_fk_id=selected.otro.id)
                            elif type_test == 2:
                                throw_asignation.objects.create(serie_fk_id=new_serie.id,athlete_fk_id=selected.otro.id)
                            selected.ready=True
                            selected.save()
                        counter+=1
                        other.pop(0)
    ...
    def get_series(compt_id):
        try:
            ...
            for i_serie in series_list:
                if type_test == 0:
                    data =[i_serie,track_asignation.objects.filter(serie_fk_id=i_serie.id)]
                elif type_test == 1:
                    data =[i_serie,jump_asignation.objects.filter(serie_fk_id=i_serie.id)]
                elif type_test == 2:
                    data =[i_serie,throw_asignation.objects.filter(serie_fk_id=i_serie.id)]
